...
<head>
    ...
    <link rel="stylesheet"
          href="<%=request.getContextPath()%>/assets/css/customer/fuel/fuelPaymentHistory-simple-style.css">
</head>
<body>
...
<%
    ...
    if (!(vehicleFilterSelectedStr == null || "all".equals(vehicleFilterSelectedStr)))
        vehicleFilterSelected = Integer.parseInt(vehicleFilterSelectedStr);
    ...
    if (!(fuelTypeFilterSelectedStr == null || "all".equals(fuelTypeFilterSelectedStr)))
        fuelTypeFilterSelected = Integer.parseInt(fuelTypeFilterSelectedStr);
    ...
    <div class="filter-section">
        <form method="get" action="<%= request.getContextPath() %>/customer/fuel/history" class="filter-form">
            <div class="filter-group">
                ...
                <input type="date" id="duration-date-start-filter" name="duration-date-start-filter"
                       value="<%= request.getParameter("duration-date-start-filter") != null ? request.getParameter("duration-date-start-filter") : "" %>">
            </div>

            <div class="filter-group">
                <label for="duration-date-end-filter">To Date:</label>
                <input type="date" id="duration-date-end-filter" name="duration-date-end-filter"
                       value="<%= request.getParameter("duration-date-end-filter") != null ? request.getParameter("duration-date-end-filter") : "" %>">
            </div>
            ...
            <div class="filter-group">
                ...
                <select name="vehicle-type-filter" id="vehicle-type-filter">
                    ...
                    <option value="<%= vehicleType %>" <%= vehicleType.equals(request.getParameter("vehicle-type-filter")) ? "selected" : "" %>><%= vehicleType %>
                    </option>
                    <%}%>
                </select>
            </div>
            ...
            <div class="filter-group">
                ...
                <select name="vehicle-filter" id="vehicle-filter">
                    ...
                    <option value="<%= vehicle.getVehicleID()%>" <%= String.valueOf(vehicle.getVehicleID()).equals(request.getParameter("vehicle-filter")) ? "selected" : "" %>><%= vehicle.getModel() %>
            ...
            <div class="filter-group">
                ...
                <select name="fuel-type-filter" id="fuel-type-filter">
                    <option value="all">All</option>
                    <%for (Fuel fuel : fuels) {%>
                    <option value="<%= fuel.getFuelID() %>" <%= String.valueOf(fuel.getFuelID()).equals(request.getParameter("fuel-type-filter")) ? "selected" : "" %>><%= fuel.getType() %>
                    </option>
                    <%}%>
                </select>
            </div>
    ...
    <div class="results-section">
        ...
        <div class="table-container">
            <table class="history-table">
                ...
                %>
                <tr>
                    <td><%= detail.getFuelPurchaseID() %>
                    </td>
                    <td><%= detail.getPurchaseDate() %>
                    </td>
                    <td><%= detail.getPurchaseTime() %>
                    </td>
                    <td><%= detail.getVehiclePlate() != null ? detail.getVehiclePlate() : "" %>
                        - <%= detail.getVehicleModel() != null ? detail.getVehicleModel() : "" %>
                    </td>
                    <td><%= detail.getFuelType() != null ? detail.getFuelType() : "" %>
                    </td>
                    <td><%= detail.getQuantity() %>
                    </td>
                    <td class="cost-display">Rs. <%= detail.getTotalCost() %>
                    </td>
                </tr>
                <%
                    }
